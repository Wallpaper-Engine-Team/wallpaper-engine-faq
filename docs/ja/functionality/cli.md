...
* **playInWindow *（オプション）*：** 一つのウィンドウの中で複数の壁紙を開いたりコントロールしたりする場合の、ウィンドウの名前です。 ウィンドウのデフォルト名は「Wallpaper Pop-out」です。
  * **width *（オプション）*：** ウィンドウの幅です。
  * **height *（オプション）*：** ウィンドウの高さです。
  * **x *（オプション）*：** ウィンドウの水平位置です。
  * **y *（オプション）*：** ウィンドウの垂直位置です。
  * **activate *（オプション）*:**ウィンドウを前景に移動させます。
  * **borderless *（オプション）*:** ウィンドウをウィンドウボーダーなしで開きます。
