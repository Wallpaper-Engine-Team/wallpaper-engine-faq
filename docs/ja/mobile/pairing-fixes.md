...
* **Androidアプリ側でお使いのPCが表示されない場合:**ファイアウォールが、端末を探知するために使われるブロードキャストのみをブロックしている可能性があります。PCのIPアドレスの直接入力をお試しください。 お持ちのモバイルデバイス上で接続の概要画面へと進み、右上にある3つの点をタップ後、**IPアドレスの手動入力**を選択してください。 お持ちのPCのIPアドレスを手動で入力してください。 Make sure Wallpaper Engine is running on your computer and that you have opened the **Connect New Device** overview, otherwise a connection is not possible.
