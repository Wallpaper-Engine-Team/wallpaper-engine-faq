...
* ** Androidアプリ側でお使いのPCが表示されない場合:** ファイアウォールが端末を探知するために使われるブロードキャストのみをブロックしている可能性があります。PCのIPアドレスを直接入力してみてください。 お持ちのモバイルデバイス上で接続の概要画面へと進み、右上にある3つの点をタップ後、**IPアドレスの手動入力**を選択してください。 お持ちのPCのIPアドレスを手動で入力してください。 PC上でWallpaper Engineが起動し、**新しいデバイスを接続**の画面が開かれている状態であることを確認してください。この状態でないと接続は実行されません。
